...
def get_random_pic(pic_type='avatar', blank=True):
    pics = ['photo/{0}/{1}.jpg'.format(pic_type, randint(1,25))]
    if blank:
        pics.append(None)
    return choice(pics)
...
for i in range(int(n/5)):
    albums.append(
                Album(title=get_random_sentence(5),
                    ...
                    cover=get_random_pic('cover'),
                    created_by_id=choice(users)['pk']
                )
            )
Album.objects.bulk_create(albums, batch_size)
albums = Album.objects.all().values('pk', 'created_by_id', 'cover')
